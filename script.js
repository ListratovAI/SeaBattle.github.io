...
let audioButton = document.createElement("button"); //кнопка для включения выключения звука
...
let firstGame = true; //дополнительная переменная, первая ли игра запущена, для реализации перезапуска игры после завершения

let audioON = false; //в каком положении кнопка включить звуки
audioButton.addEventListener("click", audioOnOff);

let runningGame = false; //запущена ли игра, необходимо что бы выключать фоновую музыку во время игры
//кнопка управления музыкой
function audioOnOff() { //вкючаем или выключаем звуки по кнопке
    if (audioON == true) {
        audioON = false;
        stopPirates();
        audioButton.innerHTML = "<p>Включить музыку</p>";
...
function SEA_BATTLE_GAME() { // функция всей игры

    if (firstGame == true) { //если игра первая то удаляем элементы стартового меню
        container.removeChild(rules);
        container.removeChild(seaBattleBottom);
        seaBattle.classList.add("seaBattleInGame");
    }

    //добавляем на страницу элементы для игры
    ...
    //подготовка поля противника, расстановка свойств объектов
    ...
    //общая функция автоматической расстановки кораблей
    function autoPlasing(fieldAuto, lengthOfShip, NumberOfShip) {
        let I = getRandom(0, 9);
        let J = getRandom(0, 9);
        let direction = getRandomBoolean(); //вертикально или горизонтально ставим
        let bad = false;
    ...
    //запускаем расстановку необходимого количества кораблей
    ...
    function removeClasses() { //удаляем классы кораблей для поля противника
        for (let i = 0; i <= 9; i++) {
            for (let j = 0; j <= 9; j++) {
    ...
    //функция для создания метки после установки корабля, но для ручной расстановки
    function intLabelUser(i, j) {
        userField[i][j].label = true;
        try {
            userField[i + 1][j].label = true;
        } catch (a) {
            console.log("не поставилось");
        }
        try {
            userField[i + 1][j + 1].label = true;
        } catch (a) {
            console.log("не поставилось");
        }
        try {
    ...
    //заполняем поле пользователя необходимыми свойствами
    ...
    //кнопки для ручной расстановки кораблей
    ...
    //функция для автоматической расстановки кораблей пользователя при нажатии на кнопку "АВТОМАТИЧЕСКИ"
    ...
    //функционал кнопки новой расстановки кораблей
    ...
    startGame.addEventListener('click', startGameFunc); //нажатие на кнопку начать игру

    function startGameFunc() {
        if (counterShips == 0) { //проверка, все ли корабли расставлены
            ...
            runningGame = true;
            stopPirates();
            let whoFirst = getRandomBoolean(); //случайно выбираем кто ходит первый
            if (whoFirst == true) {
    ...
    function colorRed() { //переключаем надпись на красную
        information.classList.add("informationRed");
        information.classList.remove("informationGreen");
    }

    function colorGreen() { //переключаем надпись на зеленую
    ...
    //общая функция для ручной расстановки кораблей
    function addShips() {
        userBad = false;
        ...
        if (userBad == false) { //если можно, то запускаем внутреннюю функцию
            addShipsInt();
        }
    ...
    //функция для удаления всех EventListenerов
    ...
    //раздача функционала кнопкам расстановки кораблей
    ...
    //массив для хранения координат в которые попали
    let shootingXYUser = [
        [],
        []
    ];

    //переменные в которых хранится сколько осталось здоровья у кораблей
    ...
    function AttackUser() {
        arrow.classList.add("arrow180"); //разворачиваем стрелку
        if (addEventListenerForAIField == false) { //если не расставленны eventListnerы, то расставляем
            for (let i = 0; i <= 9; i++) {
                for (let j = 0; j <= 9; j++) {
        ...
        //если не стреляли в клетку поля, то она подсвечивается когда наводим мышь
        ...
        function AttackUserInt() { //стреляем в клетку
            if (field[i1][j1].labelShooting == false) { //проверка, стреляли ли ранее в эту клетку
                AttackUserCheckAndPlace();
            }

            function AttackUserCheckAndPlace() {

                function eventListnerRemover() { //удаляем eventListnerы после выстрела
                    for (let i = 0; i <= 9; i++) {
                        for (let j = 0; j <= 9; j++) {
                ...
                // далее расписываем разные случаи что происходит с полем в зависимотси от того скольки палубный корабль, какой по счету корабль, какое попадание в корабль
                ...
                } else if (field[i1][j1].shipIsHere == 0) { // если не попали
    ...
    //функция для победы пользователя
    function userWin() {
    ...
    //расстановка меток на поле при потоплении корабля
    function intShootingLabel(i, j) {
        try {
            field[i + 1][j].classList.add('FailShot');
            field[i + 1][j].labelShooting = true;
        } catch (a) {
            console.log("не поставилось");
        }
        try {
            field[i + 1][j + 1].classList.add('FailShot');
        ...
        try {
            field[i][j + 1].classList.add('FailShot');
    ...
    function intShootingLabelAI(i, j) {
        ...
        try {
            userField[i + 1][j].classList.add('FailShot');
            userField[i + 1][j].labelShooting = true;
        } catch (a) {
            console.log("не поставилось");
        }
        try {
            userField[i + 1][j + 1].classList.add('FailShot');
        ...
        try {
            userField[i][j + 1].classList.add('FailShot');
    ...
    //счётчики жизней кораблей пользователя
    ...
    let needToShootToShip = false; //переменная, показывающаяя попали ли в корабль в предыдущий ход

    let needToShootToShipSecond = false; //переменная, показывающая попали ли в корабль во второй раз

    //хранение информации о первом и втором попадании в корабль
    let xCrackedShip1;
    let yCrackedShip1;
    let xCrackedShip2;
    let yCrackedShip2;
    ...
    function AttackAI() { //функция атаки противника

        function getXY() {
            if (needToShootToShip == true) { //если уже попали в корабль, то переходим к ИИ
                AI();
                return;
            } else { //если в прошлый раз не попали то берем два случайных числа и проверяем их
        ...
        function AI() { //функция для добивания раненых кораблей
            needToShootToShip = true;
            if (needToShootToShipSecond == false) { //если попали только один раз по стреляем в соседние клетки пока не попадём во второй раз
                let shootToDirection = getRandom(0, 3); //выбираем одно из четырех направлений случайно
            ...
            } else if (needToShootToShipSecond == true) { //если попали второй раз то стреляем уже по линии
                ...
                } else { //если по линии промахнулись, то возвращаемся в изначальные координаты и продолжаем стрелять в другую сторону
                    tempX = xCrackedShip2;
                    tempY = yCrackedShip2;
        ...
        //проверка, не стреляли ли в эти координаты
        ...
        //функция для разных преобразований поля в зависимости от того в какой корабль попали, какой раз попали и попали ли вообще
